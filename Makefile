...
local.input=input_small
local.output=models
local.validating=input
...
alone-prediction: jar
	spark-submit --class ${prediction.job.name} --master local[*] --conf "spark.driver.memory=5g" --conf "spark.executor.cores=4" ${jar.path} ${local.output} ${local.validating} ${local.prediction}
